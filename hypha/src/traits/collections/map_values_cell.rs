...
pub trait MapValuesCellExt<K, V>
where
    K: Hash + Eq + CellValue,
    V: CellValue,
{
    ...
    #[track_caller]
    fn map_values_cell<U, W, F>(&self, mapper: F) -> CellMap<K, U, CellImmutable>
    where
        U: CellValue,
...
impl<K, V, M> MapValuesCellExt<K, V> for CellMap<K, V, M>
where
    K: Hash + Eq + CellValue,
    V: CellValue,
{
...
mod tests {
    ...
    fn map_values_cell_reacts_per_row() {
        ...
        let out = values.map_values_cell({
    ...
    fn map_values_cell_preserves_upstream_batch_without_extra_emissions() {
        ...
        let out = source.map_values_cell(|_, v| Cell::new(*v * 10).lock());

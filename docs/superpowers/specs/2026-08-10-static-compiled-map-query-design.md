...
1. Extend allocation profiling to count steady-state allocations by operator
   shape, not only whole reactive graphs.
2. Add representative plans:
...
3. Record compile time, peak rustc RSS, artifact size, runtime latency,
   allocations, and retained bytes.
4. Add a sequential reference evaluator used by differential tests.

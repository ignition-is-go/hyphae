...
Raw outputs are checked in as `region-failstop-{focused,preflight,full-lib,clippy,fmt}.txt`, `region-failstop-steady-w{1,4}.txt`, `region-failstop-repeated-right-w1.txt`, `region-failstop-tiny-left-w1-repeat.txt`, and the two `region-failstop-baseline-32178ee-*.txt` files.

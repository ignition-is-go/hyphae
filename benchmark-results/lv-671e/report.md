...
Mechanical pairing (`validation.json`) reports:
...
The audited final two-join 100-sample CI at `5177ef5` is **2.7438–2.7666 µs**, versus v3 **4.1174–4.1352 µs**: a conservative **32.81%** improvement (`benchmark-results/lv-33ed/report.md`).

The final paired four-join measurements use exact archived checkouts, 200 samples, and a 10-second measurement window. Candidate `5177ef5` is **3.6375–3.6588 µs** (point 3.6481); the freshly rebuilt archived-v3 harness is **5.5550–5.5795 µs** (point 5.5670). Candidate point versus v3 point improves **34.47%**; candidate upper versus v3 lower conservatively improves **34.14%**. `latency/provenance.json` records both exact commands, commits, benchmark source hashes, binary hashes/sizes, paths, and `latency/*-four-join-long.txt` are the raw Criterion outputs. The v3 benchmark source hash `e9dc04a...` is also the immutable Phase-0 harness hash recorded in `docs/superpowers/plans/2026-08-10-static-map-query-phase-0-baseline.md`.
...
- `allocations/*/results.csv`: complete raw matrices.
- `allocations/*/{cells,manifest}.json`, `inputs/`, `COMPLETE`: cell identities/status/result hashes, build/matrix provenance, frozen tooling, completion sentinel.
- `compile/`: all six raw measurement JSON/stderr records and frozen adapters/runners.
- `codegen-candidate/`: exact compressed IR/ASM/objdump, scoped probe/callback, `perf.data`, report, environment, and analysis.
- `latency/`: paired 200-sample v3/candidate Criterion outputs and provenance, plus three retained candidate repetitions.
- `validate.py` and `validation.json`: reproducible mechanical completeness/correctness checks and counters.
- `sha256sums.txt`: SHA-256 for every checked-in evidence file except itself.

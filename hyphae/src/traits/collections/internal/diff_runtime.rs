...
/// Sink-driven variant of [`run_diff_runtime`] for [`MapQuery`] plan nodes.
///
...
/// Sink-driven variant of [`run_grouped_runtime`] for [`MapQuery`] plan nodes.
///
/// Used by `CountByPlan` and `GroupByPlan` to install grouping machinery
/// into a downstream sink without materializing an intermediate
/// [`CellMap`].

...
const N: usize = 32; // well over the parallel-wave threshold (8)
...
fn no_coalesce_burst_in_one_batch_keeps_events_ordered_and_serial() {
    ...
    for it in 0..ITERS {
        ...
        batch(|| {
            for i in 1..=N {
                src.set(i64::try_from(i).unwrap_or(i64::MAX));
            }
        });
        let got = received.lock().clone();
        assert_eq!(got.len(), N, "iter {it}: a no_coalesce event was dropped");
        assert_eq!(
            got, expected,
